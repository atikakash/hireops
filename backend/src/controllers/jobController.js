...
const partialJobSchema = jobSchema.fork(['title'], (schema) => schema.optional());
...
const getJob = async (req, res) => {
  try {
    ...
    return res.json({ success: true, data: { job: toJobResponse(rows[0]) } });
  } catch (err) {
...
const updateJob = async (req, res) => {
  const { error, value } = partialJobSchema.validate(req.body, {
    abortEarly: false,
    noDefaults: true,
  });
  if (error) {
  ...
  try {
    ...
    setField('department', value.department || null);
    setField('location', value.location || null);
    setField('type', value.type);
    setField('description', value.description || null);
    setField('requirements', value.requirements || null);
    if (value.is_open !== undefined) {
      setField('is_open', value.is_open ? 1 : 0);
    }

    if (fields.length) {
      params.push(req.params.id);
      await db.query(
        `UPDATE jobs SET ${fields.join(', ')}, updated_at=NOW() WHERE id=?`,
        params
      );
    }
    ...
    return res.json({
      ...
      data: { job: toJobResponse(updated[0]) },
    });
  } catch (err) {
...
const toggleJobStatus = async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, is_open FROM jobs WHERE id=? AND company_id=? AND deleted_at IS NULL LIMIT 1',
      [req.params.id, req.user.companyId]
    );
    if (!rows.length) {
    ...
    await db.query(
      'UPDATE jobs SET is_open=?, updated_at=NOW() WHERE id=?',
      [rows[0].is_open ? 0 : 1, req.params.id]
    );
...
const assignCandidateSimple = async (req, res) => {
  ...
  try {
    ...
    return assignCandidate(req, res);
  } catch (err) {
    console.error('assignCandidateSimple error:', err);
...
function toJobResponse(row) {
  const isOpen = row.is_open === true || row.is_open === 1 || row.status === 'open';
  ...
  return {
    ...
    openDate: createdAt,
    candidateCount: Number.parseInt(row.candidate_count || 0, 10) || 0,
    assignedCandidateIds: [],
    closedDate: isOpen ? null : toIso(row.updated_at || row.created_at),
    createdAt,
  };
}
